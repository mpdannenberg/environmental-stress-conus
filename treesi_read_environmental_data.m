% Add climate, topography, and soils to ITRDB Stress database
...
% Remove non-CONUS sites
tf = strncmp(site, 'ak', 2) | strncmp(site, 'cana', 4) | strncmp(site, 'mexi', 4) | strncmp(site, 'hi', 2);
TREESI(tf) = [];
clear tf site;
...
for i = 1:n
    ...
    % Soils
    TREESI(i).GSDE.BD = BD(yind, xind);
    TREESI(i).GSDE.BS = BS(yind, xind);
    TREESI(i).GSDE.CEC = CEC(yind, xind);
    ...
    TREESI(i).GSDE.VMC1 = VMC1(yind, xind);
    TREESI(i).GSDE.VMC2 = VMC2(yind, xind);
    TREESI(i).GSDE.VMC3 = VMC3(yind, xind);
    % more to come... maybe
...
save('./data/TREESI_wEnvFactors','TREESI', '-v7.3');
