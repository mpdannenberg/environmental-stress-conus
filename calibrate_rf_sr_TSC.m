...
% Relative stress index (Sr), predicted using all variables (TSC)
...
SoilImpEcoL1 = NaN(length(L1list), nSoil);
ClimImpEcoL1 = NaN(length(L1list), nTemp+nWater);

L1_stats = NaN(length(L1list), 6); % ecoregion, rmse, r2, Spearman's rho, n, nsites
L1_corrs = NaN(length(L1list), nTopo+nSoil+nTemp+nWater, 2); % correlations between RSI and env variables
...
% for i = 80:10:130
    ...
    if n>=n_min
        ...
        for s = 1:nSites
            ...
            % Soil
            X(idx:(idx+nyrs-1), nTopo+1) = treesi_sub(s).GSDE.CLAY;
            X(idx:(idx+nyrs-1), nTopo+2) = treesi_sub(s).GSDE.OC;
            X(idx:(idx+nyrs-1), nTopo+3) = treesi_sub(s).GSDE.PHCA;
            X(idx:(idx+nyrs-1), nTopo+4) = treesi_sub(s).GSDE.SAND;
            X(idx:(idx+nyrs-1), nTopo+5) = treesi_sub(s).GSDE.SILT;
            X(idx:(idx+nyrs-1), nTopo+6) = treesi_sub(s).GSDE.TN;
        ...
        SoilImpEcoL1(L1list==i, :) = B.OOBPermutedVarDeltaError((nTopo+1):(nTopo+nSoil));
        ...
        L1_stats(rowi, 5) = n;
        L1_stats(rowi, 6) = nSites;
        
        [rs,ps] = corr(X,Y, 'rows','pairwise', 'type','Spearman');
        L1_corrs(rowi, :, 1) = rs;
        L1_corrs(rowi, :, 2) = ps;
...
SoilImpEcoL2 = NaN(length(L2list), nSoil);
ClimImpEcoL2 = NaN(length(L2list), nTemp+nWater);

L2_stats = NaN(length(L2list), 6); % ecoregion, rmse, r2, Spearman's rho, n, nsites
L2_corrs = NaN(length(L2list), nTopo+nSoil+nTemp+nWater, 2); % correlations between RSI and env variables
...
for i = L2list
    ...
    if n>=n_min
        ...
        SoilImpEcoL2(L2list==i, :) = B.OOBPermutedVarDeltaError((nTopo+1):(nTopo+nSoil));
...
SoilImpEcoL3 = NaN(length(L3list), nSoil);
ClimImpEcoL3 = NaN(length(L3list), nTemp+nWater);

L3_stats = NaN(length(L3list), 6); % ecoregion, rmse, r2, Spearman's rho, n, nsites
L3_corrs = NaN(length(L3list), nTopo+nSoil+nTemp+nWater, 2); % correlations between RSI and env variables
...
for i = L3list
    ...
    if n>=n_min
        ...
        SoilImpEcoL3(L3list==i, :) = B.OOBPermutedVarDeltaError((nTopo+1):(nTopo+nSoil));
...
ax = gca;
set(ax, 'FontName','Times New Roman', 'YTick', 1:nTopo,'XLim',[-0.3 2],... 
    'YTickLabel',TopoLabs);
...
ha = tight_subplot(1,3,0.02, [0.2 0.1],[0.1 0.05]);
axes(ha(1));
boxplot(SoilImpEcoL1, 'PlotStyle','compact', 'Colors',[0.4 0.4 0.4],...
    'Orientation','Horizontal', 'Jitter',0);
ax = gca;
set(ax, 'FontName','Times New Roman', 'YTick', 1:nSoil,'XLim',[-0.2 2],... 
    'YTickLabel',SoilLabs);
n = sum(isfinite(SoilImpEcoL1(:,1)));
title(['Level 1 (','{\itn} = ',num2str(n), ')'], 'FontName','Times New Roman');
grid on;

axes(ha(2));
boxplot(SoilImpEcoL2, 'PlotStyle','compact', 'Colors',[0.4 0.4 0.4],...
    'Orientation','Horizontal', 'Jitter',0);
ax = gca;
set(ax, 'FontName','Times New Roman', 'YTick', 1:nSoil,'XLim',[-0.2 2],...
    'YTickLabel',{'','','',''});
n = sum(isfinite(SoilImpEcoL2(:,1)));
title(['Level 2 (','{\itn} = ',num2str(n), ')'], 'FontName','Times New Roman');
text(0.5,-1.4,'Mean Decrease in Accuracy when Variable is Excluded',...
    'FontName','Times New Roman', 'HorizontalAlignment','center', 'FontSize',14);
grid on;

axes(ha(3));
boxplot(SoilImpEcoL3, 'PlotStyle','compact', 'Colors',[0.4 0.4 0.4],...
    'Orientation','Horizontal', 'Jitter',0);
ax = gca;
set(ax, 'FontName','Times New Roman', 'YTick', 1:nSoil,'XLim',[-0.2 2],...
    'YTickLabel',{'','','',''});
n = sum(isfinite(SoilImpEcoL3(:,1)));
title(['Level 3 (','{\itn} = ',num2str(n), ')'], 'FontName','Times New Roman');
grid on;

set(gcf,'PaperPositionMode','auto')
print('-dtiff','-f2','-r600','SoilImportanceBox.tif')
...
boxplot(ClimImpEcoL1, 'PlotStyle','compact', 'Colors',[0.4 0.4 0.4],...
    'Orientation','Horizontal', 'Jitter',0);
ax = gca;
set(ax, 'FontName','Times New Roman', 'YTick', 1:(nTemp+nWater),'XLim',[-0.4 2],... 
...
set(gcf,'PaperPositionMode','auto')
print('-dtiff','-f3','-r600','ClimImportanceBox.tif')
...
pos = get(h, 'Position');
pos(1) = pos(1)+0.02;
set(h, 'Position',pos, 'FontName','Times New Roman', 'YTick',0:0.1:0.5);
...
cd RF_v6_RSI_TSC;
set(gcf,'PaperPositionMode','auto')
print('-dtiff','-f1','-r600','RF_v6_RSI_TSC_ValStats.tif')

save('RF_v6_RSI_TSC_stats.mat','L1_stats','L2_stats','L3_stats','L1_corrs','L2_corrs','L3_corrs','AllLabs', '-v7.3');
